...
def _validate_course_structure_payload(units_config):
...
def create_course_structure_logic(course_id: str, units_config: dict, edit: bool = False, user_identifier=None):
    ...
    try:
        ...
        created_structure = []
        validation_error = _validate_course_structure_payload(units_config)
        if validation_error:
            logger.warning(
                "create_course_structure invalid payload course_id=%s keys=%s error=%s",
                course_id,
                list(units_config.keys()) if isinstance(units_config, dict) else type(units_config).__name__,
        ...
        if not created_structure:
            logger.warning(
                "create_course_structure produced empty structure course_id=%s requested_units=%s",
                course_id,
                len(units_config.get("units", [])),
            )
            return {
                "success": False,
                "error": "no_structure_created",
                "message": "The structure request was accepted but no chapters, subsections, or units were created",
                "course_id": course_id,
                "edit_mode": edit,
                "created_structure": [],
            }
